...
public class ProdutoDAO {
	...
	public void AlterarProduto(Produto produto) throws Exception {
		try {
			String sql = "UPDATE produto SET nome=?, valor=?, categoria_idcategoria=? WHERE idproduto=?";
			PreparedStatement sqlPrep = Conexao.conectar().prepareStatement(sql);
			int contador = 1;
			sqlPrep.setString(contador++, produto.getDescricao());
			sqlPrep.setDouble(contador++, produto.getValor());
			sqlPrep.setInt(contador++, produto.getCategoria());
			sqlPrep.setInt(contador++, produto.getCodigo());
	...
	public List<Object> buscarTodos() throws SQLException, Exception {
		...
		try {
			String sql = "SELECT * FROM produto join categoria on produto.categoria_idcategoria = idcategoria;";
			java.sql.Statement state = Conexao.conectar().createStatement();
			ResultSet rs = state.executeQuery(sql);

			while (rs.next()) {
				Object[] linha = { rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(6), };
				cliente.add(linha);
			}
			state.close();
	...
	public List<Object> buscaCategoriaSelecionada(int comanda) throws SQLException, Exception {
		List<Object> produto = new ArrayList<Object>();
		try {
			String sql = "SELECT produto.idproduto, produto.nome, produto.valor FROM produto where categoria_idcategoria =" + comanda + "";
			java.sql.Statement state = Conexao.conectar().createStatement();
			ResultSet rs = state.executeQuery(sql);

			while (rs.next()) {
				Object[] linha = {rs.getInt(1), rs.getString(2), rs.getString(3),"0" };
				produto.add(linha);
			}
			state.close();
